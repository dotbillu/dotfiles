...
Rectangle {
    ...
    property string openWidget: ""  // "wifi" | "bt" | "sound" | ""

    function openOnly(name) {
        if (openWidget === name) {
            openWidget = ""
            wifiWidget.hide(); btWidget.hide(); soundWidget.hide()
        } else {
            openWidget = name
            if (name !== "wifi")  wifiWidget.hide()
            if (name !== "bt")    btWidget.hide()
            if (name !== "sound") soundWidget.hide()
            if (name === "wifi")  { wifiWidget.show() }
            if (name === "bt")    { btWidget.show() }
            if (name === "sound") { soundWidget.show() }
        }
    }
    ...
    RowLayout {
        ...
        Item {
            ...
            Rectangle {
                ...
                anchors.topMargin: 9; anchors.rightMargin: 6
        ...
        // ── Separator ────────────────────────────────────────────────────────
        Rectangle { width: 1; height: 18; color: Theme.colors.border; opacity: 0.5; Layout.alignment: Qt.AlignVCenter }
        ...
        Item {
            ...
            Text {
                ...
                text: {
                    if (root.sinkMute || root.sinkVol === 0) return "󰖁"
                    if (root.sinkVol < 0.33)                 return "󰕿"
                    if (root.sinkVol < 0.66)                 return "󰖀"
                    return "󰕾"
                }
            }
            MouseArea {
    ...
    HoverHandler {
        ...
        onHoveredChanged: {
            ...
            else if (root.openWidget !== "" && !anyWidgetHovered()) closeDelay.restart()
        }
    }

    function anyWidgetHovered() {
        return wifiWidget.popupHovered || btWidget.popupHovered || soundWidget.popupHovered
    }
    ...
    WifiWidget {
        ...
        onPopupHoveredChanged: {
            ...
            else if (!islandHover.hovered) closeDelay.restart()
        }
    }
